...
$(function () {
    ...
    var game = {
        ...
        drawMap: function () {
            ...
            mouse.handlePanning();
            context.drawImage(this.currentLevel.mapImage, this.viewportX, this.viewportY, this.viewportWidth, this.viewportHeight, this.viewportLeft, this.viewportTop, this.viewportWidth, this.viewportHeight);
        ...
        draw: function () {
            ...
            for (var i = game.buildings.length - 1; i >= 0; i--) {
                ...
                if (build.team == game.currentLevel.team) {
                    fogContext.beginPath();
                    fogContext.globalCompositeOperation = "destination-out";
                    fogContext.arc((Math.floor(build.x)) * game.gridSize + build.pixelWidth / 2, (Math.floor(build.y)) * game.gridSize + build.pixelHeight / 2, build.sight * game.gridSize, 0, 2 * Math.PI, false);
                    fogContext.fill();
                }
            }
            ;
            for (var i = game.turrets.length - 1; i >= 0; i--) {
                ...
                if (turret.team == game.currentLevel.team || turret.bulletFiring) {
                    fogContext.beginPath();
                    fogContext.globalCompositeOperation = "destination-out";
                    fogContext.arc((Math.floor(turret.x)) * game.gridSize + turret.pixelWidth / 2, (Math.floor(turret.y)) * game.gridSize + turret.pixelHeight / 2, turret.sight * game.gridSize, 0, 2 * Math.PI, false);
                    fogContext.fill();
                }
            }
            ;
            fogContext.restore();
            context.drawImage(this.fogCanvas, 0 + game.viewportX * this.canvasWidth / currentMap.width, 0 + game.viewportY * this.canvasHeight / currentMap.height, game.viewportWidth * this.canvasWidth / currentMap.width, game.viewportHeight * this.canvasHeight / currentMap.height, game.viewportLeft, game.viewportTop, game.viewportWidth, game.viewportHeight);
        }
    };
    ...
    var AStar = (function () {
        ...
        function AStar(grid, start, end, f) {
            var cols = grid[0].length, rows = grid.length, limit = cols * rows, f1 = Math.abs, f2 = Math.max, list = {}, result = [], open = [{ x: start[0], y: start[1], f: 0, g: 0, v: start[0] + start[1] * cols }], length = 1, adj, distance, find, i, j, max, min, current, next;
