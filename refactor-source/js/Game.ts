...
class Game implements IBulletDrawer {
    ...
    highlightGrid(i: number, j: number, width: number, height: number, optionalImage?: string | HTMLImageElement): void {
        ...
        if (isImage) {
            this.context.drawImage(<CanvasImageSource><unknown>optionalImage, i * gridSize + this.screen.viewportAdjust.x, j * gridSize + this.screen.viewportAdjust.y, width * gridSize, height * gridSize);
        } else {
            if (optionalImage) {
