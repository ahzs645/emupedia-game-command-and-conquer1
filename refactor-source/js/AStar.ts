...
function AStar(grid: number[][], start: [number, number], end: [number, number], f: string): IPoint[] {
    var cols = grid[0].length,
        ...
        length = 1,
        newEnd = { x: end[0], y: end[1], v: end[0] + end[1] * cols },
        fmax: number,
        adj, distance: Function, find, i: number, j: number, max: number, min: number, current, next;
    switch (f) {
        case "Diagonal":
